...
$(document).ready(function(){
	...
	if(isMobile){
		...
		$(".people").insertAfter(".transborderlab");
		$(".logo_wrap").insertAfter(".info");
		$('.whole').css({'opacity':'0'})
		$('.content_wrapper').css({'opacity':'1'})
		$('.schedule').html(' ')
		$('.schedule').append('<span class="title_f title">Schedule</span><br><span class="b_f">June 29th, 2020 <br>- July 24th, 2020<br>DMZ Trip : July 6th- 8th, 2020<br></span>')
		$('.location').html(' ')
		$('.location').append('<span class="title_f title">Location</span><br><a href="https://www.insa1kote.com/map" id="location_button" target="_blank" class="b_f">7 Insadong-gil,Insa-dong,<br>Jongno-gu,Seoul</a><br></span>')
	}
	var trans_array = [0,0]
	...
	function start(pos_x,pos_y){
			...
			if(!isMobile){
				...
				$('.side_1.wrap_1.extra').css({'transform':'rotateX( 0deg)  rotateY( 0deg) translateZ('+ (r_y) + 'px)'})
				$('.side_1.wrap_1.extra').css({'height':(r_y/2) + 'px'})
				$('.side_2.wrap_1.extra').css({'width':r_x/2 + 'px'})
				$('.side_2.wrap_1.extra').css({'top':-1*(r_y/2) + 'px'})
				$('.side_2.wrap_1.extra').css({'transform':'rotateX( 90deg)  rotateY( 0deg) translateY('+ (r_y) + 'px)'})
				...
				$('.side_3.wrap_1.extra').css({'transform':'rotateX( 0deg)  rotateY( 90deg) translateZ('+  (-1*(r_y)+r_x/2) + 'px)'})
				...
				$('.side_3.wrap_1.extra').css({'width':(r_y) + 'px'})
				...
				$('.side_1.wrap_1').css({'height':r_y + 'px'})
				...
				$('.side_1.wrap_3').css({'transform':'rotateX( 0deg) rotateY( 0deg) translateZ(-50vw)'})
				...
				$('.side_2.wrap_3 .content').css({'transform':'translateY('+(-1*r_y) + 'px)'}) 
				...
				$('.side_3.wrap_2').css({'height':r_y + 'px'})
...
function people_interaction(){
	...
	$('.lang_wrap .lang_kr').click(function(){
		...
		set_content()
				$('.side_1').html(' ')
				$('.side_1').append('<div class="gradient"></div>')
				$('.side_1').append($('.side_3.wrap_1 .content').clone());
				$(body).addClass('kr_m')
		}
	})
	function get_data(){
	 	...
	 	var xhttp = new XMLHttpRequest();
	  	xhttp.onreadystatechange = function() {
		    if (this.readyState == 4 && this.status == 200) {
				base = JSON.parse(this.responseText).values
		    }
	  	};
        xhttp.open("GET", "https://sheets.googleapis.com/v4/spreadsheets/1hiRVj4SlAoONGTvFGTf63EWOCvfFYjxJtNpaQyk5bz0/values/FSSS_default?key=AIzaSyAmcp44cOi9-6XM4EqjCjIQLbj_D__1YPE");
		xhttp.send();
		loaded()
		function loaded(){
			if(typeof base == 'undefined' || ($('.side_3 .objective').length==0)){
				setTimeout (function(){
					loaded()
				},500)
			}else{
				var sen_1 = []
				$('.side_1 .objective').html('')
				$('.side_1 .objective').append('<span class="title_f title">Objective</span><br>')
				$('.side_1 .objective').append(base[13][1])
				$('.side_1 .transborderlab').html('')
				$('.side_1 .transborderlab').append('<span class="title_f title">Transborder Lab </span><br>')
				$('.side_1 .transborderlab').append(base[14][1])
				for (var i = 1; i < base.length-2; i++) {
					sen_1.push('<span class="circle"></span>' + base[i][1])
					if(i == base.length-3){
						$('.side_1 .menifesto.b_f').html('')
                    	$('.side_1 .menifesto.b_f').append('<span class="title_f title">Manifesto</span><br>')
						$('.side_1 .menifesto.b_f').append(sen_1.join('<br>'))
						set_content()
					}
				}
				var sen_1_kr = []
				$('.side_3 .objective').html(' ')
				$('.side_3 .objective').append('<span class="title_f title">Objective</span><br>')
				$('.side_3 .objective').append(base[13][2])
				$('.side_3 .transborderlab').html('')
				$('.side_3 .transborderlab').append('<span class="title_f title">Transborder Lab </span><br>')
				$('.side_3 .transborderlab').append(base[14][2])
				for (var i = 1; i < base.length-2; i++) {
					sen_1_kr.push('<span class="circle"></span>' + base[i][2])
					if(i == base.length-3){
						$('.side_3 .menifesto.b_f').html('')
                    	$('.side_3 .menifesto.b_f').append('<span class="title_f title">Manifesto</span><br>')
						$('.side_3 .menifesto.b_f').append(sen_1_kr.join('<br>'))
						set_content()
					}
				}
			}
	...
	function get_bio() {
	 	var xhttp = new XMLHttpRequest();
	  	xhttp.onreadystatechange = function() {
		    if (this.readyState == 4 && this.status == 200) {
				bio = JSON.parse(this.responseText).values
		    }
	  	};
        xhttp.open("GET", "https://sheets.googleapis.com/v4/spreadsheets/1hiRVj4SlAoONGTvFGTf63EWOCvfFYjxJtNpaQyk5bz0/values/FSSS_bio?key=AIzaSyAmcp44cOi9-6XM4EqjCjIQLbj_D__1YPE");
		xhttp.send();
		loaded()
		function loaded(){
			if(typeof bio == 'undefined'){
				...
				for (var i = 0; i <  bio.length-1; i++) {
					console.log(bio[i][0])
					console.log(bio[i][4])
					$('.side_1 .people_elem_'+i).html(' ')
					$('.side_1 .people_elem_'+i).append(bio[i][0])
					$('.side_3 .people_elem_'+i).html(' ')
					$('.side_3 .people_elem_'+i).append(bio[i][1])
					$('.popup_en.popup_'+i+' .name').html(' ')
					$('.popup_en.popup_'+i+' .name').append(bio[i][0])
					$('.popup_en.popup_'+i+' .contents').html(' ')
					$('.popup_en.popup_'+i+' .contents').append(bio[i][2].split("<").join("&#60;").split(">").join("&#62;"))
					$('.popup_kr.popup_'+i+' .name').html(' ')
					$('.popup_kr.popup_'+i+' .name').append(bio[i][1])
					$('.popup_kr.popup_'+i+' .contents').html(' ')
					$('.popup_kr.popup_'+i+' .contents').append(bio[i][3].split("<").join("&#60;").split(">").join("&#62;"))
					if(typeof bio[i][4] !== 'undefined'){
						console.log('hdf')
						$('.popup_kr.popup_'+i+' .contents').append('<br><br><a href='+bio[i][4]+' target="_blank">Interview</a>')
						$('.popup_en.popup_'+i+' .contents').append('<br><br><a href='+bio[i][4]+' target="_blank">Interview</a>')
					}
					if(i==bio.length-2){
						set_content()
						people_interaction()
						// $('.whole_logo').addClass('whole_logo_out') 
					}
				}
			}
		}
